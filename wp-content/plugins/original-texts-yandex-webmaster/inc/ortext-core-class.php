...
// Убедимся что поле установлено.
        ...
        if ($chek0 == 'on') { //Проверка галки о публикации
            ...
            update_post_meta($post_id, '_ortext_error', $status_sent);
    ...
    public function futureSentYandex($ID, $post) {
        ...
        $status_sent = $ortextfun->sendTextOriginal2($post->post_content); //Отправка текста
        if ($status_sent == 000) {
            $post_id = 000;
            $title = 'Error plugin';
            $status_sent = 'error';
        ...
        $ortextfun->logJornal($ID, $post->post_title, $status_sent, $post->post_type); //Логируем результаты
